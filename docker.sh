...
# Get the current release channel for PlayCast
get-release-channel() {
  local PLAYCAST_VERSION="latest"
  if [[ -f .env ]]; then
    .env --file .env get PLAYCAST_VERSION
    PLAYCAST_VERSION="${REPLY:-latest}"
  fi

  echo "$PLAYCAST_VERSION"
}
...
#
# Configure the ports used by PlayCast.
#
setup-ports() {
  envfile-set "PLAYCAST_HTTP_PORT" "80" "Port to use for HTTP connections"
  envfile-set "PLAYCAST_HTTPS_PORT" "443" "Port to use for HTTPS connections"
  envfile-set "PLAYCAST_SFTP_PORT" "2022" "Port to use for SFTP connections"
}
...
setup-release() {
  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/ikafridi/PlayCast/main/sample.env -o .env
  fi

  local PLAYCAST_VERSION="latest"
  if ask "Prefer stable release versions of PlayCast?" N; then
    PLAYCAST_VERSION="stable"
  fi

  .env --file .env set PLAYCAST_VERSION=${PLAYCAST_VERSION}
}

check-install-requirements() {
  local CURRENT_OS CURRENT_ARCH REQUIRED_COMMANDS SCRIPT_DIR

  echo "Checking installation requirements for PlayCast..."
  ...
  if [[ $CURRENT_OS == "Linux" ]]; then
    ...
    echo "       Automated PlayCast installation is not currently supported on this"
  ...
  if [[ $SCRIPT_DIR == "/var/playcast" ]]; then
    echo -en "\e[32m[PASS]\e[0m Installation Directory\n"
  else
    echo -en "\e[93m[WARN]\e[0m Installation Directory\n"
    echo "       PlayCast is not installed in /var/playcast, as is recommended"
    echo "       for most installations. This will not prevent PlayCast from"
...
run-installer() {
  local PLAYCAST_RELEASE_BRANCH
  PLAYCAST_RELEASE_BRANCH=$(get-release-branch-name)

  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/ikafridi/PlayCast/$PLAYCAST_RELEASE_BRANCH/sample.env -o .env
  fi
  if [[ ! -f playcast.env ]]; then
    curl -fsSL https://raw.githubusercontent.com/ikafridi/PlayCast/$PLAYCAST_RELEASE_BRANCH/playcast.sample.env -o playcast.env
  fi
  if [[ ! -f docker-compose.yml ]]; then
    curl -fsSL https://raw.githubusercontent.com/ikafridi/PlayCast/$PLAYCAST_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.yml
  fi

  touch docker-compose.new.yml

  curl -fsSL https://raw.githubusercontent.com/ikafridi/PlayCast/$PLAYCAST_RELEASE_BRANCH/docker-compose.installer.yml -o docker-compose.installer.yml
...
#
# Run the initial installer of Docker and PlayCast.
...
install() {
  ...
  if [[ $EUID -ne 0 ]]; then
    .env --file .env set PLAYCAST_PUID="$(id -u)"
    .env --file .env set PLAYCAST_PGID="$(id -g)"
  fi
...
install-dev() {
  ...
  if [[ ! -d ../docker-playcast-nginx-proxy ]]; then
    if ask "Clone related repositories?" Y; then
      git clone https://github.com/PlayCast/docker-playcast-nginx-proxy.git ../docker-playcast-nginx-proxy
      git clone https://github.com/PlayCast/docker-playcast-nginx-proxy-letsencrypt.git ../docker-playcast-nginx-proxy-letsencrypt
      git clone https://github.com/PlayCast/docker-playcast-db.git ../docker-playcast-db
      git clone https://github.com/PlayCast/docker-playcast-redis.git ../docker-playcast-redis
      git clone https://github.com/PlayCast/docker-playcast-radio.git ../docker-playcast-radio
    fi
  fi
  ...
  if [[ ! -f playcast.env ]]; then
    ...
    echo "Customize playcast.env file now before continuing. Re-run this command to continue installation."
    exit
  fi
...
update() {
  ...
  if ask "Are you ready to continue with the update?" Y; then
    ...
    curl -fsSL https://raw.githubusercontent.com/ikafridi/PlayCast/$PLAYCAST_RELEASE_BRANCH/docker.sh -o docker.new.sh
    ...
    if [[ ! -s docker-compose.new.yml ]]; then
      curl -fsSL https://raw.githubusercontent.com/ikafridi/PlayCast/$PLAYCAST_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.new.yml
    fi
    ...
    docker volume rm playcast_www_vendor
    docker volume rm playcast_tmp_data
    docker volume rm playcast_redis_data

    docker-compose run --rm --user="playcast" web playcast_update "$@"
    docker-compose up -d
...
update-self() {
  ...
  curl -fsSL https://raw.githubusercontent.com/ikafridi/PlayCast/$PLAYCAST_RELEASE_BRANCH/docker.sh -o docker.sh
  chmod a+x docker.sh
...
#
cli() {
  docker-compose run --rm --user="playcast" web playcast_cli "$@"
  exit
}
...
#
bash() {
  docker-compose exec --user="playcast" web bash
  exit
}
...
db() {
  ...
  .env --file playcast.env get MYSQL_HOST
  MYSQL_HOST="${REPLY:-mariadb}"

  .env --file playcast.env get MYSQL_PORT
  MYSQL_PORT="${REPLY:-3306}"

  .env --file playcast.env get MYSQL_USER
  MYSQL_USER="${REPLY:-playcast}"

  .env --file playcast.env get MYSQL_PASSWORD
  MYSQL_PASSWORD="${REPLY:-azur4c457}"

  .env --file playcast.env get MYSQL_DATABASE
  MYSQL_DATABASE="${REPLY:-playcast}"
...
backup() {
  ...
  docker-compose run --rm web playcast_cli playcast:backup "/var/playcast/backups/${BACKUP_FILENAME}" "$@"

  # Move from Docker volume to local filesystem
  docker run --rm -v "playcast_backups:/backup_src" \
  -v "$BACKUP_DIR:/backup_dest" \
  busybox mv "/backup_src/${BACKUP_FILENAME}" "/backup_dest/${BACKUP_FILENAME}"
}

#
# Restore an PlayCast backup into Docker.
# Usage:
...
restore() {
  ...
  if [[ ! -f .env ]] || [[ ! -f playcast.env ]]; then
    echo "PlayCast hasn't been installed yet on this server."
  ...
  if ask "Restoring will remove any existing PlayCast installation data, replacing it with your backup. Continue?" Y; then
    docker-compose down -v

    docker volume create playcast_backups

    # Move from local filesystem to Docker volume
    docker run --rm -v "$BACKUP_DIR:/backup_src" \
      -v "playcast_backups:/backup_dest" \
      busybox mv "/backup_src/${BACKUP_FILENAME}" "/backup_dest/${BACKUP_FILENAME}"
    ...
    docker-compose run --rm web playcast_restore "/var/playcast/backups/${BACKUP_FILENAME}" "$@"
...
restore-legacy() {
  ...
  if [ -f "$BACKUP_PATH" ]; then
    ...
    docker volume rm playcast_db_data playcast_station_data
    docker volume create playcast_db_data
    docker volume create playcast_station_data

    docker run --rm -v "$BACKUP_DIR:/backup" \
      -v playcast_db_data:/playcast/db \
      -v playcast_station_data:/playcast/stations \
...
uninstall() {
  if ask "This operation is destructive and will wipe your existing Docker containers. Continue?" N; then
    ...
    echo "All PlayCast Docker containers and volumes were removed."
