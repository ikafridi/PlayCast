...
- The entire PlayCast codebase has been set to "strict mode" in PHP, which will strictly enforce type safety across the
...
- You can optionally disable Redis entirely, instead relying on flatfile caches for session management and other Redis
  functions by setting `ENABLE_REDIS=false` in `playcast.env`. This is not recommended for most users as Redis offers
...
- One of the biggest issues with Docker file mounting has been permissions; you can now set a custom UID/GID for the
  ...
  set `PLAYCAST_PUID` and `PLAYCAST_PGID` in `.env` accordingly; both default to 1000.

- All up-to-date PlayCast installations will opt users out of Google's new advertisement tracking system, FLoC. Learn
...
# PlayCast 0.13.0 (Jun 15, 2021)

## New Features/Changes

- **Podcast Management (Beta):** You can now upload and manage podcasts directly via the PlayCast web interface. Via
...
- **Automatic Theme Selection:** If you haven't set a default theme for either your user account or the PlayCast public
...
- The PHP-SPX (Simple Profiling eXtension) extension has been added and can be enabled via a new environment variable in
  `playcast.env`; this will allow very simple visual profiling of the application and its memory/CPU usage over time.
...
- PlayCast can now properly read _and write_ metadata to/from Flac and Ogg Vorbis files without issue.
...
- For sequential or shuffled playlists, you can now view the internal queue that the PlayCast AutoDJ uses to track its
...
- An issue with some stations crashing shortly after startup has been resolved. This was caused by a safety check we
  added to the AutoDJ to check that PlayCast was up and running at the same time; however, this caused issues with
  stations that don't use the PlayCast AutoDJ (i.e. stations that stream live or use remote playlists).
...
- Across all PlayCast repositories, the `master` branch has been renamed to `main`.
...
# PlayCast 0.12 (Jan 27, 2021)

This update introduces significant new features and fixes a number of bugs reported by the community.

## New Features/Changes

- **Remote Album Art Retrieval**: If enabled in the system settings panel, PlayCast will now check remote services to
  attempt to retrieve album art if it is missing, or not provided (i.e. for live DJs). By default, this system uses the
  MusicBrainz database, which is comprehensive but can be slow; if you provide an API key for the last.fm API, PlayCast
...
- **Media Manager Improvements:** Some changes have been made to the media manager to improve the user experience and
  accessibility:
    ...
    - You can now instruct PlayCast to re-analyze and reprocess the selected media files in the Media Manager.
...
- Unprocessable media is now stored in a separate database table along with the date/time processed and the relevant
  ...
  processable but are processed in every 5-minute sync. PlayCast will automatically re-check files marked as
...
- For more advanced setups, you can now set the following environment variables in `playcast.env` to use a third-party
  Redis service instead of the one bundled with PlayCast:
    - `REDIS_HOST` (default: `redis` for Docker, `localhost` for Ansible)
    - `REDIS_PORT` (default: 6379)
    - `REDIS_DB` for the database index (default: 1)

- There is a new debug CLI command, `playcast:debug:optimize-tables`, which optimizes all tables in the MariaDB
...
- Previously PlayCast's codebase had a `Settings` class and a `Settings` entity that managed two separate things, which
...
- There is a new, much friendlier animation that displays when Docker installations of PlayCast are waiting for their
...
- **Removal of InfluxDB**: Despite the ever-increasing complexity of PlayCast, sometimes we review our dependencies to
...
- **Songs table overhaul**: A `Songs` table has existed for the entirety of PlayCast's existence as the authoritative
...
# PlayCast 0.10.4 (Oct 1, 2020)

This release is being tagged to incorporate all of the feature improvements, bug fixes and other changes we've made to
PlayCast over the last few months into a stable release just prior to our Hacktoberfest event. We expect the
...
- Raspberry Pi support has been restored, and PlayCast's Ansible installation is once again compatible with Raspberry
...
- Fixed a bug where you could continue into PlayCast without finishing setup. (#2958)
...
- We have merged the configuration from our "multi-site" setup into our core Docker Compose configuration. This means
  ...
  multiple domains alongside your PlayCast installation. See issue #2855 for instructions on migrating your existing
...
- There is a new CLI command to list all accounts on an PlayCast installation. For Docker users, this can be invoked
  via `./docker.sh cli playcast:account:list`.
...
# PlayCast 0.10.2 (May 6, 2020)

This is a primarily bug fix oriented release that includes several important improvements. All users of PlayCast are
...
- You can set a default album art URL on a per-station level; if one is not set at the station level, the system-wide
  one will be used instead (and if one is not provided there, the default PlayCast album art image will be used
  instead).
...
- The PlayCast homepage will now remind you if you haven't run any sort of backup (either manual or automated nightly)
...
- The "Cued" time in the upcoming song queue will now accurately reflect when PlayCast expects the track to actually be
  played.
...
# PlayCast 0.10.0 (Apr 26, 2020)

PlayCast 0.10.0 is an incremental release with several bug fixes and improvements across the application. This version
introduces minor changes to the PlayCast API (see below) and is tagged as a new minor release as a result.
...
- PlayCast's AutoDJ will now build a queue of songs ahead of the current song. Instead of just seeing the "next" song,
...
- Several improvements have been made to scheduling and the PlayCast AutoDJ in general (#2740, #2689, #2631). This is
...
- **Schedule View**: Now that both playlists and streamers are scheduled, you can use PlayCast to produce a full
...
- **Record Live Broadcasts**: Thanks to several improvements within PlayCast, we now track the start and end times for
...
- **New IP Geolocation Options**: With the changes to the MaxMind GeoLite database, we've abstracted out how we perform
  ...
  and PlayCast will download and use the GeoLite database instead.

## Bug Fixes and Minor Improvements

- The `playcast_web_v2` Docker image is now directly tied to the main code repository, so rolling-release updates are
...
- The "Prefer Browser URL" and "Enable SFTP Server" settings have been tweaked so they will behave more sensibly, and
  will not interfere with portions of PlayCast's web interface that make API calls.

- Several sections of PlayCast (including, notably, the Duplicate Songs report and Radio Automation code) have been
...
- The PlayCast AutoDJ's scheduling code has been significantly overhauled to be easier to maintain and to fix a number
...
- When previewing a music track or other finite-length file within the PlayCast web interface, you can seek through the
...
- **Switched Built-in FTP Service to SFTP:** We encountered a number of issues with our previous built-in FTP service,
  ...
  modifiable by changing/adding `PLAYCAST_SFTP_PORT` in `.env` on the host) to handle all incoming connections, is a
...
- The mini music player inside the PlayCast interface is now inline with the header menu instead of accessed via a
  dropdown menu.
...
# PlayCast 0.9.7 (Oct 11, 2019)

## New Features

- **New Media Manager**: The Media Manager is one of the core parts of the PlayCast experience, and we want it to be as
...
- You can now customize even more of the PlayCast Docker installation parameters by modifying configuration lines in
  your local [playcast.env](https://github.com/ikafridi/PlayCast/blob/main/playcast.sample.env#L70-L80) file.

## Bug Fixes

- We have resolved a major issue with one of our third-party libraries that causes "No valid bundles" errors to appear
  any time PlayCast attempts to connect to a secure URL. This can happen even on pages that don't seemingly make any
...
- This version includes a significant number of code quality improvements under the hood. You won't notice many of these
  ...
  but they are meant to make PlayCast's code much more consistent and easier to maintain, which means faster bug fixes
  and more new features!

---

# PlayCast 0.9.6.5 (Aug 27, 2019)

## New Features

- This incremental minor version release adds support for PlayCast's built-in FTP server on Docker installations, which
  allows you to use your PlayCast credentials to log in and manage media via a high-performance FTP server.

---

# PlayCast 0.9.6.2 (Aug 27, 2019)
...
- Tweaked the PlayCast AutoDJ's duplicate artist/title prevention algorithm to be more forgiving of playlists that
...
- Fixed a scenario where hundreds of "zombie processes" were being created by PlayCast sending notification messages to
...
- When determining your external-facing IP address (which requires pinging a service that's not located on your
  installation, one of the few times we do this), we have switched to using our own PlayCast Central server for this
...
# PlayCast 0.9.6 (Jul 27, 2019)

It's only been just over a month since PlayCast 0.9.5.1 was released, but we've made some very significant, very
important updates to the software in that time, especially in the fields of reliability and performance.

## New Features

- **The PlayCast AutoDJ is Back**: A few versions ago, we had attempted to switch entirely to using Liquidsoap for our
  ...
  PlayCast AutoDJ include:
    - Being able to see the next song that will play in the queue,
    - Applying playlist changes without reloading all of PlayCast, and
    - Avoiding songs playing back-to-back with either the same title _or_ the same artist using our duplicate prevention
      system.

- **Introducing AzuraRelay**: This version of PlayCast has built-in support for our new "relay-in-a-box"
  software [AzuraRelay](https://github.com/PlayCast/AzuraRelay). Simply drop AzuraRelay onto a server that can run
  Docker, answer a few quick questions, and it will automatically connect to PlayCast, detect all stations, and relay
  them all. It will also list itself as a relay on the "host" PlayCast station, and report back listener and client
  details as well.
...
- **Two New NowPlaying API Sources**: Our most powerful and comprehensive set of data is all compiled into what we call
  ...
  new [Now Playing Data APIs Guide](https://docs.playcast.com/en/developers/apis/now-playing-data).
...
- The currently playing song is written atomically to `/var/playcast/stations/(station_name)/nowplaying.txt`, which is
  particularly useful for our [radio-video-stream](https://github.com/PlayCast/radio-video-stream) example project.
...
- If you are leaving a page with a form on it that has unsaved changes, you will be prompted to confirm leaving the
  page.
- Web servers will now more aggressively cache the hashed static assets used in PlayCast, improving performance.
...
- The Highcharts library has been completely replaced across the entire application. This is largely because of
  ...
  component from early PlayCast development that required updating to be a fully free and open-source stack.
...
- Fixed a number of scenarios that caused the "This station is powered by PlayCast" jingle to play despite a station
...
# PlayCast 0.9.5 (May 14, 2019)

Work on PlayCast never stops, and with the project increasing in popularity, we've been even busier than before. A lot
...
- **API Parity**: We've done a _lot_ of work to make all of the core functionality of PlayCast available via our REST
  ...
  visit `your-playcast-url/api` for API documentation specific to your installed version that you can test in-browser
...
- Fixed #1405 and other bugs related to the new Liquidsoap AutoDJ process that prevented PlayCast from getting proper "
...
- If PlayCast detects that you've modified your installation locally (specifically your Ansible/"Traditional"
...
- When restarting a station, the PlayCast error message won't play immediately, but it will play if your station has
  nothing to play.
...
PlayCast AutoDJ mode have been merged. Liquidsoap handles the scheduling and playback of all "once every x" and
otherwise scheduled playlists, and PlayCast's AutoDJ handles, by default, the "general rotation" playlists. If an error
occurs with PlayCast's AutoDJ, Liquidsoap will manage general rotation playlists, too.
...
We have also made improvements to how Liquidsoap is notified of playlist changes from PlayCast, so these notifications
...
- **"Jingle Mode" Playlists**: You can now tell PlayCast to ignore metadata updates for a given playlist, so that when
...
- **Support for Two-Factor Authentication**: Now any user on PlayCast can significantly improve their security by
  ...
  the [password reset instructions](https://github.com/ikafridi/PlayCast/blob/main/SUPPORT.md#reset-an-account-password)
...
- The Twitter web hook now has support for rate limiting, so you can tell PlayCast to only send one Tweet every few
...
- The installation method formerly known as "Traditional" has been renamed to "Ubuntu Bare-Metal" installation. The
  term "traditional" conveyed to many people that this was the "normal" or preferred way of installing PlayCast, when
...
- Further improvements have been made to the process of notifying PlayCast of song changes, and the station profile
...
- Several new protections have been introduced to prevent users from deleting themselves, removing critical permissions
  ...
  running `[your cli command] playcast:account:set-administrator [your-email-address]`.
...
- The `station_watcher` secondary process has been removed on Docker installations, replaced by new scripting
  ...
  support for PlayCast's rich metadata even when using "Manual AutoDJ" mode or "Advanced" playlists.
...
- Starting with this release, releases are now tracked on the Composer package
  manager's [Packagist repository](https://packagist.org/packages/playcast/playcast). We will be using Packagist to
...
- **Redesigned Station Profiles and Per-Station File Quotas**: The form you see when editing your station profile has
  been divided into new tabs for easier use. If you are a global administrator of a multi-tenant PlayCast instance, you
...
- **Release-Only Updates**: If you prefer to update less frequently, you can now choose to only update to the latest
  tagged release of PlayCast instead of using the typical "rolling release" schedule. Both `./update.sh --release`
...
- When viewing your API documentation (`your-playcast-site/api`), the documentation will allow you to test the
...
- In some previous versions, PlayCast referred to a third-party CDN (CloudFlare's CDNJS) for many of its static assets
  for performance reasons. From this version forward, those assets have been moved back to the PlayCast instance
  itself. This helps to preserve the self-contained nature of PlayCast and avoid unnecessary downtime caused by
...
# PlayCast 0.9.0 (Oct 8, 2018)

**Important note:** PlayCast is a "rolling-release" product that is updated almost daily. If you're running PlayCast,
...
- **SHOUTcast 2 DNAS is no longer bundled with PlayCast.** While SHOUTcast has been a popular offering that was bundled
  with PlayCast after significant demand, it has always been non-free, proprietary software, the only component in the
  PlayCast stack that is not free and open-source. With the release of SHOUTcast's
  new ["freemium" pricing structure](https://www.shoutcast.com/Pricing) has also come a new, more aggressive license
  associated with distribution of the software. As such, PlayCast can no longer bundle SHOUTcast 2 DNAS with new
...
- **Our support for Icecast is now even better suited for commercial radio stations.** Along with our withdrawal of
  ...
  version of PlayCast, you will now see much more accurate information on your listeners, especially if you're using
...
- **The core "Now Playing" library is a standalone PHP library now.** We're always looking for new ways to give back to
  the open-source software community. Now, along with [AzuraForms](https://github.com/PlayCast/playforms), we have
  spun off our [NowPlaying](https://github.com/PlayCast/nowplaying) library to be a standalone component you can
...
- **PlayCast now has early support for plugins.** Many of you have wanted to customize the internal workings of
  PlayCast without needing to fork the main codebase and maintain your own copy. You can now do this with the help of
  our new plugin architecture, which lets you hook directly into the most important events that happen "under the hood"
  in PlayCast without modifying the core code. Check out
  our [example plugin](https://github.com/PlayCast/example-plugin) for a demonstration of what plugins can do.
...
- **PlayCast has a new homepage!** Visit our simple yet informative new homepage, powered by VuePress,
  at [playcast.com](https://www.playcast.com/).
...
- When editing a station's profile, PlayCast will automatically check if a port is in use by another station before
...
# PlayCast 0.8.0 (Dec 15, 2017)

This release includes the cumulative development work done over the fall and winter of 2017 on the PlayCast project. It
...
- **Broadcast to remote streams:** Running your AutoDJ from PlayCast but broadcasting to a hosted radio server
...
- **Frontend dependencies relocated to CDNs**: Many of the frontend dependencies used by the web app have been moved and
  ...
  significantly lightened the size of the PlayCast codebase.
