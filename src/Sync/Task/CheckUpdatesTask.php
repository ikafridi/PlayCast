...
class CheckUpdatesTask extends AbstractTask
{
    ...
    public function __construct(
        ...
        protected PlayCastCentral $playcastCentral,
    ...
    public function run(bool $force = false): void
    {
        ...
        if (Environment::getInstance()->isTesting()) {
            $this->logger->info('Update checks are currently disabled for this PlayCast instance.');
            return;
        }

        try {
            $updates = $this->playcastCentral->checkForUpdates();
            ...
            if (!empty($updates)) {
                ...
                $this->logger->error('Error parsing update data response from PlayCast central.');
            }
        } catch (TransferException $e) {
            $this->logger->error(sprintf('Error from PlayCast Central (%d): %s', $e->getCode(), $e->getMessage()));
            return;
        }
