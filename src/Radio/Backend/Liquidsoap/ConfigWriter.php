...
class ConfigWriter implements EventSubscriberInterface
{
    ...
    public function writeHeaderFunctions(WriteLiquidsoapConfiguration $event): void
    {
        if ($event->isForEditing()) {
            $event->prependLines(
                [
                    '# ' . __('Welcome to the PlayCast Liquidsoap configuration editor.'),
                    '# ' . __('Using this page, you can customize several sections of the Liquidsoap configuration.'),
                    '# ' . __('The non-editable sections are automatically generated by PlayCast.'),
                ]
            );
        } else {
            $event->prependLines(
                [
                    '# WARNING! This file is automatically generated by PlayCast.',
        ...
        playcast_api_auth = ref "${stationApiAuth}"
        ignore(playcast_api_auth)
    ...
    /**
     * Returns the URL that LiquidSoap should call when attempting to execute PlayCast API commands.
     *
    ...
    protected function getApiUrlCommand(Entity\Station $station, string $endpoint, array $params = []): string
    {
        ...
        if ($this->environment->isDocker()) {
            $params = (array)$params;
            $params['api_auth'] = '!playcast_api_auth';
            ...
            $shell_args = [];
            $shell_args[] = 'playcast:internal:' . $endpoint;
    ...
    public function writeHarborConfiguration(WriteLiquidsoapConfiguration $event): void
    {
        ...
        def dj_auth(auth_user,auth_pw) =
            ...
            ret = {$authCommand}
            log("PlayCast DJ Auth Response: #{ret}")
        ...
        def live_connected(header) =
            ...
            ret = {$djonCommand}
            log("PlayCast Live Connected Response: #{ret}")
        end
        ...
        def live_disconnected() =
            ...
            ret = {$djoffCommand}
            log("PlayCast Live Disconnected Response: #{ret}")
    ...
    public function writePreBroadcastConfiguration(WriteLiquidsoapConfiguration $event): void
    {
        ...
        $event->appendBlock(
            <<<EOF
        # Send metadata changes back to PlayCast
        def metadata_updated(m) =
            def f() =
                if (m["song_id"] != "") then
                    ret = {$feedbackCommand}
                    log("PlayCast Feedback Response: #{ret}")
                end
                (-1.)
            end
